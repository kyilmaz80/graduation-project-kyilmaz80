...
USE `upod_otel`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `upod_otel`.`ReservationView` AS select `r`.`id` AS `id`,`r`.`room_id` AS `room_id`,`r`.`checkin_date` AS `checkin_date`,`r`.`checkout_date` AS `checkout_date`,`r`.`checkedin_time` AS `checkedin_time`,`r`.`checkedout_time` AS `checkedout_time`,`c`.`full_name` AS `customer_name` from ((`upod_otel`.`Reservation` `r` join `upod_otel`.`ReservationCustomer` `rc` on((`r`.`id` = `rc`.`reservation_id`))) join `upod_otel`.`Customer` `c` on((`rc`.`customer_id` = `c`.`id`)));
